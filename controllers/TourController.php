...
class TourController extends MyController
{
        ...
        */
        ...
        public function actionTh_tour($view = 'normal', $month = '', $fg = '', $status = '') {
            ...
            if (isset($_POST['tour_id'])) {
                ...
                if (isset($_POST['kq'])) $theTourStat->qhkh_ketthuc = $_POST['kq'];
                if (isset($_POST['kt'])) $theTourStat->qhkh_khaithac = implode(',', $_POST['kt']);
                if (isset($_POST['point'])) $theTourStat->qhkh_diem = $_POST['point'];
    ...
    public function actionFeedback($id = 0, $action = 'add', $feedback = 0)
    {
        ...
        if ($action == 'remove' && $feedback != 0) {
            Yii::$app->db->createCommand()->delete('fb', ['id'=>$feedback])->execute();
            return $this->redirect(Yii::$app->request->referrer);
        }
        $ver = '20072017';
        if ($feedback > 0) {
            $theFb = Fb::find()->where(['id' => $feedback])->one();
            if (!$theFb) {
                throw new HttpException(404, 'Fb not found.');
            }
            $ver = $theFb['version'];
        }else {
            $theFb = new Fb();
        ...
        if (isset($_POST['save'])) {
            $tables = [];
            $total_diem = [];
            foreach ($versions[$ver]['questions'] as $kq => $q) {
                $arr_table = [];
                if (isset($_POST[$kq])) {
                    foreach ($_POST[$kq] as $k => $table) {
                        if (is_numeric($k)) {
                            foreach ($table as $op => $v) {
                                if (in_array($op, $q['options'])) {
                                    if ($v > 0) {
                                        $op_value = $v - 1;
                                    }
                                    else {
                                        $op_value = count($q['v_op_v']) - 1;
                                    }
                                    $arr_table[$k]['options'][] = $op_value;
                                    $key = array_search($op, $q['options']);
                                    $arr_table[$k]['diem'][] = $q['v_op_v'][$key][$op_value];

                                }
                            }
                        } else {
                            if (in_array($k, $q['options'])) {
                                if ($table > 0) {
                                    $op_value = $table - 1;
                                }
                                else {
                                    $op_value = count($q['v_op_v'][0]) - 1;
                                }
                                $arr_table['options'][] = $op_value;
                                $key = array_search($k, $q['options']);
                                if (!isset($q['v_op_v'][$op_value][$op_value])) {
                                    $op_value = (string)$op_value;
                                }
                                $arr_table['diem'][] = $q['v_op_v'][$key][$op_value];
                            }
                        }
                    }
                    if (!empty($arr_table)) {
                        $group = [];
                        foreach ($arr_table as $key => $q) {
                            if (is_numeric($key)) {
                                $arr_table[$key]['total_q'] = array_sum($q['diem']);
                                $group[] = array_sum($q['diem']);
                            } else {
                                $arr_table['total_q'] = array_sum($arr_table['diem']);
                                if ($key == 'diem') {
                                    $total_diem[] = array_sum($arr_table['diem']);
                                }
                            }
                        }
                        if (!empty($group)) {
                            $total_diem[] = array_sum($group) / count($group);
                        }
                        if (isset($_POST[$kq]['note_q'])) {
                            $arr_table['note_q'] = $_POST[$kq]['note_q'];
                        }
                        $tables['questions'][$kq] = $arr_table;
                    }
                }
            }
            $tables['total'] = array_sum($total_diem);
            // var_dump($tables['questions']['q2']);die;
            $score = 0;
            $arr_score = [1 => '0,419', 2 => '420,599', 3 => '600,799', 4 => '800,1099', 5 => '1100'];
            foreach ($arr_score as $k => $str_score) {
                $ex_score = explode(',', $str_score);
                if (count($ex_score) == 2) {
                    if ($tables['total'] >= $ex_score[0] && $tables['total'] <= $ex_score[1]) {
                        $score = $k;
                        break;
                    }
                }
                if (count($ex_score) == 1) {
                    if ($tables['total'] >= $ex_score[0]){
                        $score = $k;
                        break;
                    }
                }
            ...
            $theFb->ct_id = $theTour['id'];
            $theFb->version = $ver;
            $theFb->content = $content;
            if ($score == 0) {
                var_dump($tables['total']);die;
            }
            $theFb->score = $score;
            if ($theFb->save(false)) {
                return $this->redirect('/tours/feedback?id='.$theTour['id'].'&action=update&feedback='.$theFb['id']);
            }
        }
            ...
            foreach ($db_data['questions'] as $q => $q_content) {
                if ($q !== 'q2' && $q !== 'q3') {
                    ...
                    foreach ($q_content as $t => $t_content) {
                        if (!isset($t_content['diem'])) continue;
            ...
            if (isset($_POST['action']) && isset($_POST['dvtour_id']) && isset($_POST['tour_id']) && isset($_POST['formdata']))
            {
                ...
                if ($_POST['action'] == 'update') {
                    ...
                    if ($_POST['action'] == 'vat') {
                        ...
                        $vatStatus = '';
                        if ($s['vat_ok'] == '')  $vatStatus = 'needed';
                        if ($s['vat_ok'] == 'needed')  $vatStatus = 'ok';
                        if ($s['vat_ok'] == 'ok')  $vatStatus = '';
