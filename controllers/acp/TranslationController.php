...
class TranslationController extends \app\controllers\MyController
{
    ...
    public function actionFields($lang = '', $cat = '')
    {
        ...
        $sysHelpTrans = Translation::find()
            ...
            ->all();
        // ACC translations
        $accLabelTrans = Translation::find()
            ->select(['cat', 'source', 'translation'])
            ->where(['account_id'=>ACCOUNT_ID, 'cat'=>'field_label', 'language'=>$lang])
            ->indexBy('source')
            ->asArray()
            ->all();
        $accHelpTrans = Translation::find()
            ->select(['cat', 'source', 'translation'])
            ->where(['account_id'=>ACCOUNT_ID, 'cat'=>'field_help', 'language'=>$lang])
            ->indexBy('source')
            ->asArray()
            ->all();
        ...
        if (Yii::$app->request->isPost) {
            if (isset($_POST['label'], $_POST['help'])
                ...
                foreach ($_POST['label'] as $key=>$val) {
                    if (!isset($accLabelTrans[$key]) && $val != '') {
                        // Insert
                    ...
                    } elseif (isset($accLabelTrans[$key]) && $val != $accLabelTrans[$key]) {
                        // Update
                ...
                foreach ($_POST['help'] as $key=>$val) {
                    ...
                    } elseif (isset($accHelpTrans[$key]) && $val != $accHelpTrans[$key]) {
                        // Update
                        Yii::$app->db->createCommand()->update('translations', [
                            'updated_dt'=>NOW,
                            'updated_by'=>USER_ID,
                ...
                return $this->redirect('/acp/translations');
            }
        }
        ...
        return $this->render('translation_fields', [
            ...
            'accLabelTrans'=>$accLabelTrans,
            'accHelpTrans'=>$accHelpTrans,
        ]);
    }
    ...
    public function actionLists($id = null, $lang = '')
    {
        ...
        if (Yii::$app->request->isPost) {
            if (isset($_POST['list']) && isset($_POST['lang'])
                ...
                return $this->redirect('/acp/translations/lists');
            }
        }
    ...
    public function actionMessages($cat = '', $lang = '')
    {
        ...
        // Single cat
        $sysTransFile = Yii::getAlias('@app').'/messages/'.$lang.'/'.$cat.'.php';
        if (!file_exists($sysTransFile)) {
            throw new HttpException(404, 'Source language messages not found.');
        }
        $sysMessages = include($sysTransFile);

        $accTransFile = Yii::getAlias('@app').'/messages/'.$lang.'/account'.ACCOUNT_ID.'/'.$cat.'.php';
        if (!file_exists($accTransFile)) {
            $accMessages = [];
        } else {
            $accMessages = include($accTransFile);
        }

        if (Yii::$app->request->isPost && isset($_POST['message']) && is_array($_POST['message'])) {
            foreach ($sysMessages as $source=>$trans) {
                if (isset($_POST['message'][md5($source)]) && $_POST['message'][md5($source)] != '') {
                    // If there is acc trans and it's not blank, use it
                    $accMessage[addslashes($source)] = addslashes($_POST['message'][md5($source)]);
                } else {
                    // Otherwise use default sys trans
                    $accMessage[addslashes($source)] = addslashes($trans);
                }
            }
            ...
            ksort($accMessage);
            $content = "<?php\n\nreturn [";
            foreach ($accMessage as $source=>$trans) {
                $content .= "\n    '$source'=>'$trans',";
            }
            $content .= "\n];";
            ...
            if (!file_exists($accTransFile)) {
                // @\yii\helpers\FileHelper::createDirectory($accTransFile);
            //     \yii\helpers\FileHelper::createDirectory($accTransFile);
            }
            $accFile = fopen($accTransFile, "w") or die("Unable to open file!");
            fwrite($accFile, $content);
            fclose($accFile);
            // \fCore::expose($accMessage);
            // exit;
            Yii::$app->session->setFlash('success', Yii::t('a', 'Translations have been updated.'));
            return $this->redirect('/acp/translations/messages');
        }
