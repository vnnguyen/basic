...
class ListitemController extends \app\controllers\MyController
{
	public function actionU($id = 0)
	{
		...
		$theList = Listt::find()
			//->where(['account_id'=>ACCOUNT_ID, 'id'=>$theItem['list_id']])
			->where(['id'=>$theItem['list_id']])
			->with([
				'items'=>function($q) {
		...
		if ($theItem->load(Yii::$app->request->post()) && $theItem->validate()) {
			...
			$theItem->save(false);
			return $this->redirect('/acp/lists/r/'.$theItem['list_id']);
		}
		return $this->render('//acp/list/listitem_u', [
			'theItem'=>$theItem,
			'theList'=>$theList,
		]);
	}
