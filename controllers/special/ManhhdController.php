...
class ManhhdController extends \app\controllers\MyController
{
    // Thong ke thu / chi tour theo thang ket thuc
    public function actionIndex($view = 'enddate', $year = 2017, $orderby = 'enddate')
    {
        $sql = 'SELECT COUNT(*) AS total, (SELECT DATE_ADD(day_from, INTERVAL day_count-1 DAY)) AS end_date
            FROM at_ct WHERE op_status="op" GROUP BY SUBSTRING(end_date,1,7) HAVING YEAR(end_date)=:year ORDER BY MONTH(end_date)';

        $tourData = Yii::$app->db->createCommand($sql, [':year'=>$year])->queryAll();

        foreach ($tourData as $tour) {
            $y = (int)substr($tour['end_date'], 0, 4);
            $m = (int)substr($tour['end_date'], 5, 2);
            $results[$y][$m] = $tour['total'];
        }

// \fCore::expose($results); exit;
        return $this->render('manhhd_index', [
            'results'=>$results,
            ]);
    }
