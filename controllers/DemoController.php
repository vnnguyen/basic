...
class DemoController extends MyController
{
    ...
    public function actionFloyd2(){
        $soDinh = 7;
        $start = 5;
        $distanc = [];// khoang cach cacs diem
        $t = []; // dinh thu 3 giua 2 diem
        $visities = [3,2,7]; // cac diem can tham
        ...
        for ($i = 1; $i <= $soDinh ; $i++) {
         ...
         $distanc[2][7]= $distanc[7][2]= 2;
         $distanc[3][7]= $distanc[7][3]= 6;
        // var_dump($distanc); die();

         
        $getData = $this->floyd($distanc, $t, $soDinh);
        $distanc = $getData['d'];
        $t = $getData['t'];
         function path($t_param, $u, $v, $path_param = []){
            // $path = $v;
            // $path_param[] = $path;
            // while ($path != $u && $path > 0) {
            //     $path = $t_param[$u][$path];
            ...
            // }
            if ($t_param[$u][$v] !== -1) {
                $path_param[] = $dinh = $t_param[$u][$v];
                if ($t_param[$u][$dinh] !== -1) {
                    path($t_param, $u, $dinh, $path_param);
                } else if ($t_param[$dinh][$v] !== -1) {
                    path($t_param, $dinh, $v, $path_param);
                }
            } 
            // krsort($path_param);
            return $path_param;
        ...
        $not_yet_visited = $visities;
         function point($start, $not_yet_visited, $T_nongDo, $distanc){
            
            $uv = $not_yet_visited;
            $p = p($start, $uv, $T_nongDo, $distanc);
            $r = rand(1, 10) / 10;
            $t = 0;
            $i = 0;
            while ($t < 0.8 ) {
        ...
        function p($u, $uv, $t, $distanc){
            $sum = [];
            foreach($uv as $k =>$v){
                $sum[$k] = $distanc[$u][$v] > 0? $t[$u][$v] * (1/ $distanc[$u][$v]): 0;
            }
            $total = array_sum($sum);
            foreach($sum as $k =>$v){
                $p[$k] = $v / $total;
        ...
        for($i = 1; $i <= $soDinh; $i ++){
            for($j = 1; $j <= $soDinh; $j ++){
                $T_nongDo[$i][$j] = 0.5;// nong do
                $delta[$i][$j] = 0;//khoi tao vet
                $T_nongDo[$j][$i] = $T_nongDo[$i][$j]; $delta[$j][$i] = $delta[$i][$j];
            }
        }
        $stop_loop = 1000;
        ...
        do {
            ...
            for ($i = 1; $i <= $traveler ; $i++) { 
                ...
                while (!empty($not_yet_visited) && $start > 0) {
                    // chon diem tiep theo
                    if (in_array($start, $not_yet_visited)) {
                        $not_yet_visited = array_values(array_diff($not_yet_visited,[$start]));
                        if(empty($not_yet_visited)) break;
                    }
                    $next = point($start, $not_yet_visited, $T_nongDo, $distanc); //rand($j, $soDinh);
                    
                    $cost = $cost + $distanc[ $start ][ $next ];
                    
                    echo $start.'-'. $next. ' <br>';
                    $visited[] = $next;
                    $not_yet_visited = array_values(array_diff($visities, $visited));
                    $start = end($visited);
                ...
                if ($cost > 0){
                    for($i = 1; $i <= $soDinh-1; $i ++){
                        for($j = $i + 1; $j <= $soDinh; $j ++){
                            ...
                            $T_nongDo[$i][$j] = 0.8 * $delta[$i][$j] + $q/$cost;
                            $T_nongDo[$j][$i] = $T_nongDo[$i][$j];
                        }
                    }
                }
        ...
        // end next point
        var_dump($best_router);
        var_dump($cost_best);die;
        $p = array_merge([2], $visited);
        $lichTrinh = [];
        foreach ($p as $k => $v) {
            if(isset($visited[$k + 1])){
                $lichTrinh[] = $p[$k];
                $path = path($t, $p[$k], $p[$k + 1]);
                if (!empty($path)) {
                    foreach($path as $u) {
                        $lichTrinh[] = $u;
                    }
    ...
    public function actionFloyd1(){
        $soDinh = 7;
        $start = 5;
        $distanc = [];// khoang cach cacs diem
        $t = []; // dinh thu 3 giua 2 diem
        $visities = [3,6, 7, 4]; // cac diem can tham
        $visited = [];// cac diem da tham
        $not_yet_visited = [];//cac diem chua tham
        $INF = 100000000;
