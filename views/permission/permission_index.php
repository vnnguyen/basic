...
Yii::$app->params['page_title'] = 'Permissions';
    ...
    <div class="panel panel-default">
        ...
        <table class="table table-condensed table-striped table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-muted text-center"><?= $permission['id'] ?></td>
                    <td><?= Html::a($permission['name'], 'permissions/u/'.$permission['id']) ?></td>
                    <td><?= $permission['alias'] ?></td>
                    <td><?= $permission['info'] ?></td>
                    <td><?= $permission['id'] ?></td>
                    <td>
                        <?= Html::a('<i class="fa fa-trash-o"></i>', 'permissions/d/'.$permission['id'], ['class'=>'text-muted']) ?>
                    </td>
                </tr>
                <? } ?>
            </tbody>
...
<div class="col-md-12">
    <form method="get" action="" class="well well-sm form-inline">
        ...
        <select class="form-control w-auto" name="month">
            ...
            <? foreach ($startDateList as $permission) { ?>
            <option value="<?= $permission['ym'] ?>" <?= $getMonth == $permission['ym'] ? 'selected="selected"' : ''?>><?= $permission['ym'] ?></option>
            <? } ?>
        </select>
    ...
    <div class="table-responsive">
        <table class="table table-striped table-condensed table-bordered table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-muted text-center"><?= strtoupper($permission['language']) ?> | <?= strtoupper($permission['offer_type']) ?></td>
                    <td>
                        <i class="fa fa-file-text-o popovers pull-right text-muted"
                            ...
                            data-content="
                        <?
                        $dayIds = explode(',', $permission['day_ids']);
                        if (count($dayIds) > 0) {
                            $cnt = 0;
                            foreach ($dayIds as $id) {
                                foreach ($permission['days'] as $day) {
                                    if ($day['id'] == $id) {
                                        $cnt ++;
                        ...
                        ?>
                        "></i>
                        <? if ($permission['offer_count'] == 0) { ?><?= Html::a('+', 'ct/propose/'.$permission['id'], ['title'=>'+ New proposal']) ?><? } else { ?>
                        <?= !isset($permission['cases'][0]) ?: Html::a('<i class="fa fa-briefcase"></i>', 'cases/r/'.$permission['cases'][0]['id'], ['class'=>'text-warning', 'title'=>'View case: '.$permission['cases'][0]['name']]) ?>
                        <?= Html::a($permission['tour']['code'], 'tours/r/'.$permission['tour']['id'], ['title'=>'View tour: '.$permission['tour']['name'], 'style'=>'background-color:#ffc; color:#060; padding:0 5px;']) ?>
                        <? } ?>
                        <?= Html::a($permission['title'], 'ct/r/'.$permission['id']) ?>
                        <span class="text-muted"><?= $permission['about'] ?></span>
                    </td>
                    <td><?= $permission['day_from'] ?></td>
                    <td class="text-center"><?= count($permission['days']) ?></td>
                    <td class="text-center"><?= $permission['pax'] ?></td>
                    <td class="text-right"><?= number_format($permission['price'], 0) ?> <span class="text-muted"><?= $permission['price_unit'] ?></span></td>
                    <td><?= Html::a($permission['updatedBy']['name'], 'users/r/'.$permission['updatedBy']['id']) ?></td>
                    <td>
                        <?= Html::a('<i class="fa fa-edit"></i>', 'ct/u/'.$permission['id'], ['class'=>'text-muted', 'title'=>'Edit']) ?>
                        <?= Html::a('<i class="fa fa-trash-o"></i>', 'ct/d/'.$permission['id'], ['class'=>'text-muted', 'title'=>'Delete']) ?>
                    </td>
                </tr>
                <? } ?>
            </tbody>
