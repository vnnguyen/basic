...
if ($theCase['stats']) {
	if ($theCase['stats']['destinations'] != '') {
		$this->params['small'] = $theCase['stats']['destinations'].' ';
		$this->params['small'] .= $theCase['stats']['pax_count_min'].'p ';
		$this->params['small'] .= $theCase['stats']['day_count_min'].'d ';
		$this->params['small'] .= date('j/n/Y', strtotime($theCase['stats']['avail_from_date']));
	}
}
...
<div class="col-md-6">
	...
	<ul class="media-list note-list">
		<li class="media note-list-item">
			<?= Html::a(Html::img('/timthumb.php?zc=1&w=100&h=100&src='.Yii::$app->user->identity->image, ['class'=>'note-author-avatar']), '@web/users/r/'.Yii::$app->user->id, ['class'=>'media-left hidden-xs']) ?>
...
?>
		<li class="media note-list-item">
			<?= Html::a(Html::img($userAvatar, ['class'=>'note-author-avatar']), '@web/users/r/'.$note['from']['id'], ['class'=>'media-left hidden-xs']) ?>
			<?
			$title = $note['title'];
			$body = $note['body'];
			/*
...
?>
		<li class="media note-list-item">
			...
			<?= Html::a(Html::img($userAvatar, ['class'=>'note-author-avatar']), '#', ['class'=>'media-left hidden-xs']) ?>
			<div class="media-body note-content">
				<h5 class="media-heading note-heading">
...
?>
		<li class="media note-list-item">
			...
			<a class="media-left hidden-xs"><i class="fa fa-lock text-info fa-3x note-author-avatar"></i></a>
