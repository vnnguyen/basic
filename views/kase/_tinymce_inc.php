...
<div id="clickToWrite" class="click-toggle" style="display:none;">
	<div style="height:44px;"><a href="#" onclick="$('.click-toggle').toggle(); return false;">Click here to start writing</a></div>
</div>
<div id="tinymceDiv" class="click-toggle">
	...
	<form id="form-tinymce" method="post" action="">
		...
		<input type="hidden" name="rtype" value="case"/>
		<input type="hidden" name="rid" value="<?= $theCase['id'] ?>"/>
		<input type="hidden" name="rname" value="<?= $theCase['name'] ?>"/>
		<input type="hidden" name="rurl" value="https://my.amicatravel.com/cases/r/<?= $theCase['id'] ?>"/>
		...
		<div class="row mb-1em">
			...
			<div class="col-md-3 text-right"><?= Html::a('Cancel', '#', ['class'=>'text-danger', 'onclick'=>'$(\'.click-toggle\').toggle(); return false;']) ?></div>
			<div class="col-md-12">
...
TXT;
$this->registerJsFile('/assets/js/plupload/js/plupload.full.js', ['depends'=>'app\assets\MainAsset']);
$this->registerJs($js);
?>
