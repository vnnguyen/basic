...
$eventVenueList = ArrayHelper::map($venueList, 'id', 'venue');

Yii::$app->params['page_layout'] = '-t';
Yii::$app->params['body_class'] = 'sidebar-xs';
Yii::$app->params['page_title'] = 'Lịch sử dụng phòng họp, tuần từ '.$thisWeek.' ('.count($theTasks).' lượt)';
...
$atList = [
    ...
    'hn'=>'Hanoi office',
    'sg'=>'Saigon office',
    'lp'=>'Luang Prabang office',
];

?>
<div class="col-md-12">
    <div class="card panel panel-default">
        <div class="card-block panel-heading">
            <form action="" class="form-inline">
            <strong>Week agenda (<?= count($theTasks) ?>)</strong>
            |
            Week from
            ...
            <?= Html::submitButton('Go', ['class'=>'btn btn-primary btn-outline']) ?>
            |
            <?= Html::a('Previous week', '?at='.$at.'&date='.$prevWeek) ?>
            |
            <?= Html::a('Next week', '?at='.$at.'&date='.$nextWeek) ?>
            |
            <?= Html::a('This week', '?at='.$at) ?>
            |
            <?= Html::a('+Add', '?action=add', ['class'=>'text-pink', 'target'=>'_blank']) ?>
            |
            <?= Html::a('+Register', '?action=reg', ['class'=>'text-pink', 'target'=>'_blank']) ?>
            </form>
        </div>
            ...
            <table class="table table-striped table-narrow">
                ...
                <tbody>
                    ...
                    foreach ($theTasks as $task) {
                        ...
                        if ($ymdNow != '' && $ymdNow != $ymd) {
                            foreach ($theEvents as $event) {
                                if (substr($event['from_dt'], 0, 10) == $ymdNow) {
                                    $textClass = 'primary';
                                    if ($event['status'] == 'draft') {
                                        $textClass = 'muted';
                                    } elseif ($event['status'] == 'off') {
                                        $textClass = 'danger';
                                    }
                    ?>
                    <tr class="info">
                        <td></td>
                        <td><?= date('H:i', strtotime($event['from_dt'])) ?></td>
                        <td><i class="fa fa-check-circle text-<?= $textClass ?>"></i> <?= Html::a($event['name'], '?action=edit&id='.$event['id']) ?></td>
                        <td class="text-center"><?= $event['attendee_count'] ?></td>
                        <td><?= $event['createdBy']['name'] ?></td>
                        <td colspan="3"><?= $eventVenueList[$event['venue']] ?? $event['venue'] ?></td>
                        <td colspan="3"><?= $event['info'] ?></td>
                    </tr>
                    <?
                                }
                            }
...
$('#date').on('change', function(){
    ...
    var val = $(this).val();
    if (val != '') {
        location.href = '/tools/phonghop?at=' + loc + '&date=' + val;
    }
});
