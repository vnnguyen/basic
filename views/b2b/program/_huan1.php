...
<div class="card">
            ...
            <thead>
                <tr>
                    ...
                    <th class="no-padding-left">
                        Activity
                        <? if (in_array(USER_ID, [$theProgram['created_by'], $theProgram['updated_by']])) { ?> 
            ...
            <tbody id="sortable" style="overflow:auto">
                ...
                foreach ($dayIdList as $dayId) {
                    if (isset($theDays[$dayId])) {
                        $day = $theDays[$dayId];
                        $dayDate = date('Y-m-d', strtotime('+ '.$cnt.' days', strtotime($theProgram['day_from'])));
                        $cnt ++;
                ?>
    ...
    var start_date = '<?= $theProgram['day_from'] ?>';
    </script>
<?
$js = <<<'TXT'
$('.x').hide();
