...
foreach ($dayIdList as $di) {
    ...
    foreach ($allDays as $k=>$v) { ?>
                    <tr id="day<?= $k ?>" class="info">
                        ...
                        <td><?= in_array(USER_ID, [1]) ? '<a class="dvt-c" href="#cpt-c" day="'.$k.'">+New</a>' : '' ?></td>
                    </tr>
...
$('#okle').on('click', 'tr a.cpt-u', function(){
    ...
    $.post('/tours/ajax', {
        ...
        if (data[0] == 'NOK') {
            ...
            $('#form-cpt').find('[type=text]:first').focus();
            $('[name=dvtour_day]').val(data[1]);
            $('[name=dvtour_name]').val(data[3]);
            $('[name=oppr]').val(data[4]);
            $('[name=venue_id]').select2('val', data[15]);
            $('[name=adminby]').val(data[16]);
            $('[name=start]').val(data[17]);
            $('[name=number]').val(data[18]);
            $('[name=via_company_id]').select2('val', data[19]);
            $('[name=by_company_id]').select2('val', data[20]);
            $('[name=qty]').val(data[5]);
            $('[name=unit]').val(data[6]);
            $('[name=price]').val(data[7]);
            $('[name=unitc]').val(data[8]);
            $('[name=vat]').val(data[10]);
            $('[name=prebooking]').val(data[11]);
            $('[name=payer]').val(data[12]);
            $('[name=status]').val(data[13]);
            $('[name=due]').val(data[14]);
            $('[name=plusminus]').val(data[21]);
            $('[name=mm]').val('');
            action = 'update';
            currentday = data[1];
        }
