...
<? $form = ActiveForm::begin(); ?>
    <? if ($action == 'add') { ?>
    <p><strong>NEW TOUR GUIDE INFO</strong></p>
    <? } ?>

    <? if ($action == 'addtime') { ?>
    <p><strong>NEW SERVICE TIME FOR <?= $theGuide['guide_name'] ?></strong></p>
    <? } ?>

    <? if ($action == 'edit') { ?>
    <p><strong>EDIT INFO</strong>
        <? if ($action == 'edit' && $theGuide['guide_user_id'] != 0) { ?>
        <em class="text-danger">Note: You cannot edit guide's name. To replace guide, delete and then add new.</em>
        <? } ?>
    </p>
    <? } ?>
