...
if ($theProduct->isNewRecord) {
	Yii::$app->params['page_title'] = 'New tour product (B2C)';
	if (Yii::$app->request->get('b2b') == 'yes') {
		Yii::$app->params['page_title'] = 'New tour product (B2B)';
	}
	Yii::$app->params['page_breadcrumbs'][] = ['New', 'products/c'];
} else {
	$this->title = 'Edit: '.$theProduct['title'];
	$this->params['breadcrumb'][] = ['View', 'products/r/'.$theProduct['id']];
	$this->params['breadcrumb'][] = ['Edit', 'products/u/'.$theProduct['id']];
}
...
<div class="col-md-8">
	...
	<? } ?>
	<div class="row">
		<div class="col-md-6"><?= $form->field($theProduct, 'title') ?></div>
		<div class="col-md-6"><?= $form->field($theProduct, 'about') ?></div>
	</div>
	<?= $form->field($theProduct, 'tags') ?>
	<div class="row">
		<div class="col-md-3"><?= $form->field($theProduct, 'offer_type')->dropdownList($productTypeList, ['prompt'=>'- Select -'])->label('Tour type') ?></div>
		<div class="col-md-3"><?= $form->field($theProduct, 'language')->dropdownList($languageList, ['prompt'=>'- Select -'])->label('Language') ?></div>
		<div class="col-md-1">Days<br><?= $theProduct['day_count'] ?></div>
		<div class="col-md-2"><?= $form->field($theProduct, 'pax') ?></div>
		<div class="col-md-3" class="df"><?= $form->field($theProduct, 'day_from') ?></div>
	</div>
	<?= $form->field($theProduct, 'intro')->textArea(['rows'=>3]) ?>

	<p><strong>PRICES AND PROMOTIONS</strong></p>
	<?= $form->field($theProduct, 'prices')->textArea(['rows'=>15, 'style'=>'font-family:Courier New, mono; font-size:14px; color:#000;']) ?>
	<div class="row">
		<div class="col-md-3"><?= $form->field($theProduct, 'price') ?></div>
		<div class="col-md-3"><?= $form->field($theProduct, 'price_unit')->dropdownList(['EUR'=>'EUR', 'USD'=>'USD', 'VND'=>'VND']) ?></div>
		<div class="col-md-3"><?= $form->field($theProduct, 'price_for')->dropdownList(['personne'=>'personne', 'groupe'=>'groupe']) ?></div>
		<div class="col-md-3"><?= $form->field($theProduct, 'price_until') ?></div>
	</div>
