            ...
            <table class="table table-narrow table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php foreach ($staffList as $staffId) { if (!empty($result[$year][$m]['vn'][$staffId]) || !empty($result[$year][$m]['la'][$staffId]) || !empty($result[$year][$m]['kh'][$staffId]) || !empty($result[$year][$m]['Other'][$staffId])) { ?>
                            <div class="text-nowrap" style="border-bottom:1px solid #eee"><?= Html::a($result[$year][$m][$destCode][$staffId] ?? '-', '/tours?orderby=startdate&time='.$year.'-'.str_pad($m, 2, '0', STR_PAD_LEFT).'&departure='.$destCode.'&cservice='.$staffId, ['target'=>'_blank']) ?></div>
                            <?php } } ?>
                            <div class="text-nowrap"><?= Html::a($result[$year][$m][$destCode][0] ?? '-', '/tours?orderby=startdate&time='.$year.'-'.str_pad($m, 2, '0', STR_PAD_LEFT).'&departure='.$destCode.'&cservice=no', ['target'=>'_blank']) ?></div>
                        </td>
                        <?php } ?>
                        <td>-</td>
                    </tr>
...
Độ dài ngày tour        từ 1-8          từ 8-12         từ 13-21        từ 22
