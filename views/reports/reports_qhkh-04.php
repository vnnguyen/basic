                    ...
                    <tr>
                        ...
                        <td>
                            <?php foreach ($staffList as $staffId) {
                                ...
                                ?>
                            <div class="text-nowrap" style="border-bottom:1px solid #eee">
                                <?= Html::a($tour_count, '/tours?fg=f&orderby=startdate&time='.$year.'-'.str_pad($m, 2, '0', STR_PAD_LEFT).'&departure='.$destCode.'&cservice='.$staffId, ['target'=>'_blank', 'title'=>Yii::t('x', 'View tours')]) ?>
                                <span class="text-pink pull-right" title="<?= Yii::t('x', 'Points') ?>"><?= $total_point ?></span>
                            </div>
                            <?php } } ?>
                            <div class="text-nowrap">
                                <?= Html::a($unknown, '/tours?fg=f&orderby=startdate&time='.$year.'-'.str_pad($m, 2, '0', STR_PAD_LEFT).'&departure='.$destCode.'&cservice=no', ['target'=>'_blank', 'title'=>Yii::t('x', 'View tours')]) ?>
                                <span class="text-pink pull-right" title="<?= Yii::t('x', 'Points') ?>"><?= $total_point_unknown ?></span>
                            </div>
                        </td>
                        <?php } ?>
                        <td>
                            <?php foreach ($results_point as $user_id => $points) {?>
                            <div class="text-nowrap font-weight-bold" style="border-bottom:1px solid #eee">
                                <?= Html::a(array_sum($results_tour[$user_id]), '/tours?fg=f&orderby=startdate&time='.$year.'-'.str_pad($m, 2, '0', STR_PAD_LEFT).'&cservice='.$user_id, ['target'=>'_blank']) ?>
                                <span class="text-pink pull-right" title="<?= Yii::t('x', 'Points') ?>"><?= array_sum($points) ?></span>
                            </div>
                            <?php } ?>
                            <?php
                            ...
                            <div class="text-nowrap font-weight-bold">
                                <?= Html::a(array_sum($result_unknown_tour), '/tours?fg=f&orderby=startdate&time='.$year.'-'.str_pad($m, 2, '0', STR_PAD_LEFT).'&cservice=no', ['target'=>'_blank']) ?>
                                <span class="text-pink pull-right" title="<?= Yii::t('x', 'Points') ?>"><?= $total_unknown ?></span>
                            </div>
                        </td>
                    </tr>
                        <?php } ?>
