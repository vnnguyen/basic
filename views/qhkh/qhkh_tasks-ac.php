...
Yii::$app->params['page_breadcrumbs'] = [
    ['Customers', 'customers'],
    ['Customer care tasks', 'customers/tasks'],
    ['Customer receiving'],
];
...
Yii::$app->params['page_title'] = Yii::t('x', 'Customers\' visits and mini conferences');
Yii::$app->params['page_small_title'] = Yii::t('x', 'week starting {date}', ['date'=>$thisWeek]);

?>
<div class="col-md-12">
    <form action="" class="form-inline mb-2">
        Week starting <input type="text" id="date" style="width:120px;" class="form-control" name="date" value="<?= $thisWeek ?>">
        at <?= Html::dropdownList('at', $at, $atList, ['class'=>'form-control', 'style'=>'width:140px;']) ?>
...
?>
                            <?php if (stripos($task['description'], 'conf') !==  false) { ?><div><span class="badge badge-success">Conference BT</span></div><?php } ?>
                        </td>
...
?>
                        ...
                        <td class="text-nowrap"><?
                            ...
                            ?><div><?= Html::a($user['name'], '@web/users/'.$user['id'], ['onclick'=>'$("tr.qhkh").removeClass("info"); $("tr.qhkh'.$user['id'].'").toggleClass("info");return false;']) ?></div><?
                            }
                        ?></td>
                        ...
                        <td><?
                            ...
                            ?><div><?= $tg['guide_user_id'] == 0 ? $tg['guide_name'] : Html::a($tg['guide_name'], '/contacts/'.$tg['guide_user_id']) ?></div><?
                                }
                            }
                        ?></td>
                        <td>
...
$('#date').on('change', function(){
    ...
    var val = $(this).val();
    if (val != '') {
        location.href = '/qhkh/tasks-ac?at=' + loc + '&date=' + val;
    }
});
