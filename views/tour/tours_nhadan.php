					...
					if ($currentDay > 0 && $currentDay <= $monthDayCount[$mo]) {
						...
						foreach ($theCptx as $cpt) {
							if (
								...
								$stay .= '<div>('
									.round($cpt['qty'], 0).') '
									.Html::a($cpt['tour']['code'], '@web/tours/services/'.$cpt['tour']['id'].'#dvtour-'.$cpt['dvtour_id'], ['rel'=>'external', 'title'=>$cpt['dvtour_name'].' | '.$cpt['updatedBy']['name'].' @'.\app\helpers\DateTimeHelper::convert($cpt['updated_at'], 'j/n/Y H:i')])
									.'</div>';
							}
							// All activities
...
<?
						...
						foreach ($theWaits as $wait) {
							if (substr($wait['from_dt'], 0, 10) == $theDay) {
								echo '<div title="'.$wait['username'].' @'.\app\helpers\DateTimeHelper::convert($wait['created_at'], 'j/n/Y H:i').'">';
								// 161207 Duc Anh xoa Hoang Lan 
								if ($wait['created_by'] == USER_ID || ($wait['created_by'] == 29212 && in_array(USER_ID, [1, 8162]))) {
