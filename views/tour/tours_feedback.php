...
$dataFb = [];
if ($theFb) {
	$dataFb = unserialize($theFb['content']);
}
$action = Yii::$app->request->get('action', 'add');
if ($action != 'add') {
	if (isset($dataFb['questions']['q2'])) {
		$ques_2 = $dataFb['questions']['q2'];
		$guide = 0;
		foreach ($ques_2 as $in => $val) {
			if (is_numeric($in)) {
				$guide ++;
			}
		}
	}
	if (isset($dataFb['questions']['q3'])) {
		$ques_3 = $dataFb['questions']['q3'];
		$drivers = 0;
		foreach ($ques_3 as $in => $val) {
			if (is_numeric($in)) {
				$drivers ++;
			}
		}
	...
	<div style="">
		...
		<form id="fbForm" method="POST" accept-charset="utf-8">
			...
			?>
				...
				<h2>Partie <?= ++ $partCnt ?> : <?= $q['title']?></h2>
				<?php
				if ($action != 'add') {
					if (!isset($dataFb['questions'][$num_q])) {
						continue;
					}
				}
				if ($num_q == 'q2' && $guide > 0) {
					...
					<table class="table table-condensed table-bordered">
						...
						<? foreach ($q['options'] as $index => $op) {
							$arr_v = [];
							$current_v = count($q['options_value']);
							if ($action != 'add') {
								$arr_v = $dataFb['questions'][$num_q][$i]['options'];
								$current_v = (isset($arr_v[$index]))? $arr_v[$index] + 1: '';
							}
						?>
						<tr>
							<td class="ta-c">
								<?= Html::input('hidden', $num_q.'['.$i.']'.'['.$op.']', $current_v)?>
								<?= $op ?>
							</td>
							<?php for($j = 0; $j < count($q['options_value']); $j ++) {
								$checked = '';
								if (isset($arr_v[$index]) && $arr_v[$index] == $j){
										$checked = 'checked';
								}
								?>
								...
								<div class="checkbox">
									...
									<?php endif ?>
									<?php if ($action == 'view' && isset($arr_v[$index]) && $arr_v[$index] == $j): ?>
										<span class="selected_icon"><i class="fa fa-check fa-2" aria-hidden="true"></i></span>
									<?php endif ?>
								</div>
								</td>
							<?}?>
					...
					<?php if ($q['note_q'] != ''): 
							$note_q = '';
							if ($action != 'add') {
								if (isset($dataFb['questions'][$num_q]['note_q'])) {
									$note_q = $dataFb['questions'][$num_q]['note_q'];
								}
							}
					?>
						...
						<?php endif ?>
						<?php if ($action == 'view'): ?>
							<?= Html::textarea($num_q.'[note_q]', $note_q, ['class' => 'form-control', 'disabled'=> true])?>
						<?php endif ?>
						</div>
					<?php endif ?>
					<?
				...
				<table class="table table-condensed table-bordered">
					...
					<? foreach ($q['options'] as $index => $op) {
						...
						$current_v = '';
						if ($action != 'add') {
							$arr_v = $dataFb['questions'][$num_q]['options'];
							$current_v = (isset($arr_v[$index]))? $arr_v[$index] + 1: '';
						}
					 ?>
					<tr>
						...
						<td class="text-center selected"><div class="checkbox">
							...
							<?php endif ?>
							<?php if ($action == 'view' && isset($arr_v[$index]) && $arr_v[$index] == $i): ?>
								<span class="selected_icon"><i class="fa fa-check fa-2" aria-hidden="true"></i></span>
							<?php endif ?>
						</div>
						</td>
			...
			<?php endforeach ?>
			<?php if ($action != 'view'): ?>
				<div class="text-right"><button id="saveForm" class="btn btn-primary" type="submit" name="save">Save feedback</button></div>
			<?php endif ?>
		</form>
	</div>
</div>
