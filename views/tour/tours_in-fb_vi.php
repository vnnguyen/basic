...
<html lang="fr">
	<head>
	<meta charset="utf-8">
	<title><?= $theTour['op_code'] ?> - PHIẾU THĂM DÒ Ý KIẾN KHÁCH HÀNG - <?= $printName ?></title>
	<style>
	...
	<div style="max-width:800px;">
		<div style="width:260px; float:left; margin:0 32px 32px 0;">
			...
			<h1>
				...
				<br />PHIẾU THĂM DÒ Ý KIẾN KHÁCH HÀNG (<?= $theForm['regionName'] ?>)
			</h1>
		</div>
		<p><strong>Kính thưa quý khách</strong></p>
		...
		<table style="clear:both; margin:0;">
			<tr>
				...
				<td width="">Họ tên quý khách: <strong><?= $theForm['paxName'] ?></strong></td>
			</tr>
		</table>
		
		<div style="clear:both; margin-bottom:32px;"></div>

		<h2>Phần <?= ++ $partCnt ?> : Ý KIẾN VỀ LỊCH TRÌNH TOUR</h2>
...
?>
		<h2>Phần <?= ++ $partCnt ?> : Ý KIẾN VỀ HƯỚNG DẪN VIÊN</h2>
		<p>Xin cho biết nhận xét tổng thể về độ hài lòng của quý khách với hướng dẫn viên của tour?</p>
...
<?
	foreach ($guideNames as $guideName) {
		...
		if ($guideName != '') {
?>
		<div><strong>HƯỚNG DẪN VIÊN: <?= strtolower($guideName) == 'yes' ? '' : Html::encode($guideName) ?></strong></div>
		<table class="">
			<tr>
				<th class="ta-c">Your remarks</th>
				<td class="ta-c" width="15%">Rất tồi</td>
				<td class="ta-c" width="15%">Tồi</td>
				<td class="ta-c" width="15%">Chấp nhận được</td>
				<td class="ta-c" width="15%">Tốt</td>
				<td class="ta-c" width="15%">Rất tốt</td>
			</td>
<?
			foreach (['Ngôn ngữ', 'Sự linh hoạt', 'Kiến thức', 'Sự đúng giờ', 'Thái độ phục vụ'] as $item) {
?>
...
?>
		<h2>Phần <?= ++ $partCnt ?> : Ý KIẾN VỀ XE VÀ LÁI XE</h2>
...
?>
		...
		<h2>Phần <?= ++ $partCnt ?> : CÁC Ý KIẾN KHÁC</h2>
		...
		<p>Ngày: ______________________________________ &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Chữ ký:__________________________________</p>
		<br />&nbsp;
		<br />&nbsp;
		<br />&nbsp;
		<br />&nbsp;
		...
		<div class="ta-c">
			...
			<br />Tầng 3, toà nhà Nikko, số 27 phố Nguyễn Trường Tộ, quận Ba Đình, Hà Nội, Việt Nam
