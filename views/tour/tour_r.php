...
include_once('tour_r__u-message.php');
...
if (strtoupper(substr($theTour['op_code'], 0, 1)) == 'G') {
...
foreach ($tourGuides as $guide) {
    if (strtotime(substr($guide['use_from_dt'], 0, 10)) <= $date && $date <= strtotime(substr($guide['use_until_dt'], 0, 10))) {
        ...
        echo ($guide['gender'] == 'male' ? 'Mr ' : ($guide['gender'] == 'female' ? 'Ms ' : '')).$guide['guide_name'];
        echo '</div>';
    }
}
...
?>
                        <tr>
                            ...
                            <td class="no-padding-left">
                                ...
                                <?= $pax['contact_id'] == 0 ? Html::a($pax['name'], '/tours/pax/'.$theTour['id'].'?action=view&pax='.$pax['id']) : Html::a($pax['name'], '#@web/users/r/'.$pax['contact_id'], ['title'=>$pax['data']['pp_name'].' / '.$pax['data']['pp_name2']]) ?>
                                <?
